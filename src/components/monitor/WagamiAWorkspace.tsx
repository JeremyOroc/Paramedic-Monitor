...
function AlarmBadge({ display, locale }: Pick<WagamiAWorkspaceProps, 'display'> & { locale: 'fr' | 'en' }) {
  const text = getWagamiAText(locale)
  const alarmText = display.alarms.length > 0
    ? `${text.alarm} · ${display.alarms.join(' / ').toUpperCase()}`
    : text.noAlarm
  return (
    <div role="status" className={`absolute right-2 top-2 z-[60] rounded border bg-wagami-a-screen/95 px-2 py-1 font-sans text-[clamp(9px,0.9cqw,13px)] font-semibold ${display.alarms.length > 0 ? 'border-wagami-a-alarm text-wagami-a-alarm' : 'border-wagami-a-border text-wagami-a-muted-text'}`}>
      {alarmText}
    </div>
  )
}

function ViewFrame({ title, onBack, backLabel, children }: { title: string; onBack: () => void; backLabel: string; children: React.ReactNode }) {
  return (
    <section className="grid h-full min-h-0 grid-rows-[clamp(42px,5cqw,68px)_minmax(0,1fr)] overflow-hidden bg-wagami-a-screen text-wagami-a-text">
      <header className="flex items-center gap-3 border-b border-wagami-a-border bg-wagami-a-surface px-[clamp(8px,1.1cqw,16px)]">
        ...
        <h1 className="font-sans text-[clamp(15px,1.7cqw,25px)] font-semibold">{title}</h1>
      </header>
...
export function WagamiAWorkspace({
  ...
  if (controller.view === 'twelveLead') {
    ...
    content = (
      <ViewFrame title={text.twelveLeadTitle} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <ViewFrame title={text.etco2Title} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <ViewFrame title={text.medicationsTitle} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <ViewFrame title={text.eventLog} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <ViewFrame title={text.vitalLogTitle} onBack={controller.goBack} backLabel={text.back}>
        <div className="grid h-full grid-rows-[minmax(0,1fr)_52px] p-4">
    ...
    content = (
      <ViewFrame title={text.configureTitle} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <ViewFrame title={text.pniTitle} onBack={controller.goBack} backLabel={text.back}>
    ...
    content = (
      <CallerInfoModal open info={callerInfo} onCallerEvent={() => {}} buttonState={{ acknowledge: { disabled: true }, arrival: { disabled: true }, transport: { disabled: true } }} fullScreen contained variant="assignment" onBack={controller.goBack} route={dispatchRoute} mapReadOnly locale={controller.preferences.locale} />
    )
  }

  return <div className="relative h-full w-full">{content}<AlarmBadge display={display} locale={controller.preferences.locale} /></div>
}
