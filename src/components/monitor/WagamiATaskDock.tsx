...
export type WagamiATask = 'twelveLead' | 'etco2' | 'medications' | 'callInfo' | 'printCapture' | 'configure'
...
const TASKS: ReadonlyArray<{ key: WagamiATask; label: string }> = [
  ...
  { key: 'printCapture', label: 'Imprimer / capturer' },
...
function TaskIcon({ task }: { task: WagamiATask }) {
  ...
  if (task === 'printCapture') return <svg viewBox="0 0 24 24" aria-hidden="true" className={common}><path d="M6 8V3h12v5M6 17H4V9h16v8h-2M7 14h10v7H7zM16 11h2" /></svg>
...
export function WagamiATaskDock({ onTask }: WagamiATaskDockProps) {
  return (
