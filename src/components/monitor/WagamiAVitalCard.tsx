...
export function WagamiAVitalCard({ channel, label, value, unit, onReading }: WagamiAVitalCardProps) {
  const card = (
    <>
      ...
      {channel === 'pni' && <span className="font-sans text-[clamp(9px,0.9cqw,13px)] text-wagami-a-muted-text">Mesure PNI</span>}
    </>
  )
  ...
  if (channel === 'pni') {
    return (
      <button
        data-testid="wagami-a-vital-pni"
        type="button"
        aria-label="Démarrer une mesure PNI"
        disabled={!onReading}
        title={!onReading ? 'Disponible en phase A4' : undefined}
        onClick={onReading}
        className={cn(classes, 'disabled:cursor-not-allowed enabled:hover:bg-wagami-a-surface-raised focus-visible:outline-2 focus-visible:outline-wagami-a-pni')}
      >
        {card}
      </button>
    )
